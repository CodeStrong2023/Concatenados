...
# Definimos un función
def mi_funcion(): # Para identificar a la función utilizamos paréntesisi
...
show(person[0], person[1]) # Pasamos uno por unos los datos de la lista a la función
...
def listarNombres(*nombres): # Normalente se ultiliza: *args
    for nombre in nombres: # Se ve a convertir en una tupla
        print(nombre)
...
listarTerminos(IDE= 'Integrated Develoment Enviromment', PK='Primaruy Key')
...
desplegarNombres((10, 11)) # La convertumos a una tupla, en un solo elemento no olvidar la coma
...
print(f'El factorial del número {numeroFactorial} es: {resultado}') # Tarea que el usuario ingrese el número para calcular el factor
